...
read -r SERVER_URL
yq --inplace ".SERVER-URL = \"${SERVER_URL}\"" ${TOOLTESTER_CONFIG_FILE_NAME}

echo "Please enter your dockstore token"
echo "Note: This token must have admin or curator access to ${SERVER_URL}"
read -r TOKEN
yq --inplace ".TOKEN = \"${TOKEN}\"" ${TOOLTESTER_CONFIG_FILE_NAME}

echo "Please enter your AWS profile name (ie. fhembroff)"
read -r AWS_PROFILE
yq --inplace ".AWS-AUTHORIZATION = \"${AWS_PROFILE}\"" ${TOOLTESTER_CONFIG_FILE_NAME}

END_OF_WES_URL="ga4gh/wes/v1"

# SET WDL WES Endpoint URL
WDL_WES_ENDPOINT_URL=$(agc context describe ${WDL_CONTEXT_NAME} --format json 2> /dev/null | jq .WesEndpoint.Url | tr --delete '"')
yq --inplace ".WDL-WES-URL = \"${WDL_WES_ENDPOINT_URL}${END_OF_WES_URL}\"" ${TOOLTESTER_CONFIG_FILE_NAME}

# SET CWL WES Endpoint URL
CWL_WES_ENDPOINT_URL=$(agc context describe ${CWL_CONTEXT_NAME} --format json 2> /dev/null | jq .WesEndpoint.Url | tr --delete '"')
yq --inplace ".CWL-WES-URL = \"${CWL_WES_ENDPOINT_URL}${END_OF_WES_URL}\"" ${TOOLTESTER_CONFIG_FILE_NAME}
...
Determine_compute_environment_names_and_set_variables() {
  local NUMBER_OF_ENGINES_IN_CONTEXT=$(yq ".contexts.${CONTEXT_NAME}.engines | length" "${AGC_CONFIG_FILE}" | grep --count --invert-match ^$)
  ...
  local NUMBER_OF_MATCHING_COMPUTE_ENVIRONMENTS=$(echo "${BATCH_COMPUTE_ENVIRONMENT_NAME}" | grep --count --invert-match ^$)
  ...
  local ECS_CLUSTER_ARN=$(aws ecs list-clusters | jq ".clusterArns[] | select(.|test(${BATCH_COMPUTE_ENVIRONMENT_NAME}))" | tr --delete '"')

  local NUMBER_OF_MATCHING_ECS_ARNS=$(echo "${ECS_CLUSTER_ARN}" | grep --count --invert-match ^$)
  ...
  ECS_CLUSTER_NAME_NO_QUOTES=$(echo "${ECS_CLUSTER_NAME}" | tr --delete '"')
  # Same as above comment
  # Determines if ECS container insights are enabled, and if they are not enabled, then they are turned on
  ARE_CONTAINER_INSIGHTS_ENABLED=$(aws ecs describe-clusters --cluster "${ECS_CLUSTER_NAME_NO_QUOTES}" --include SETTINGS | jq '.clusters[0].settings[]|select(.name ==  "containerInsights") | .value' | tr --delete '"')

  if [[ "${ARE_CONTAINER_INSIGHTS_ENABLED}" != "enabled" ]]
  then
    echo "We are turning on container insights for this cluster: ${ECS_CLUSTER_NAME}"
    aws ecs update-cluster-settings --cluster "${ECS_CLUSTER_NAME_NO_QUOTES}" --settings name=containerInsights,value=enabled
  fi

  yq --inplace ".${FIELD_TO_MODIFY_IN_YAML} = ${ECS_CLUSTER_NAME}" ${TOOLTESTER_CONFIG_FILE_NAME}
}
