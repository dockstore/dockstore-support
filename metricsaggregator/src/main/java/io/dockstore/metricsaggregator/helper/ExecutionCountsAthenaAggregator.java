...
public abstract class ExecutionCountsAthenaAggregator extends RunExecutionAthenaAggregator<TimeSeriesMetric> {
    ...
    /**
     * Create an aggregator that computes a time series with the specified number of bins (representing a consecutive series of intervals), with the youngest (last) bin overlapping the specified date.
     * @param binCount the number of bins (intervals) to aggregate
     * @param now the instant which is included in the youngest bin
     */
    public ExecutionCountsAthenaAggregator(MetricsAggregatorAthenaClient metricsAggregatorAthenaClient, String tableName, int binCount, Instant now) {
    ...
    /**
     * Get the time series interval.
     */
    protected abstract TimeSeriesMetric.IntervalEnum getInterval();

    /**
     * Compute the start of the bin that's the specified number of bins in the past from the bin with the specified start.
     */
    protected abstract ZonedDateTime pastBinStart(ZonedDateTime binStart, int delta);

    /**
     * Compute the start of the bin that's the specified number of bins in the future from the bin with the specified start.
     */
    protected abstract ZonedDateTime futureBinStart(ZonedDateTime binStart, int delta);

    /**
     * Compute the start of the bin that includes the specified date/time.
     */
    protected abstract ZonedDateTime overlappingBinStart(ZonedDateTime zonedDateTime);

    private ZonedDateTime getBinStart(int binAge) {
        ZonedDateTime zonedNow = ZonedDateTime.ofInstant(now, ZONE_ID);
        ZonedDateTime binZeroStart = overlappingBinStart(zonedNow);
        return pastBinStart(binZeroStart, binAge);
    }
